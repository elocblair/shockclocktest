...
uint8_t FRAMflag = 0;
uint8_t readFRAMflag = 0;
APP_TIMER_DEF(data_dump_id);

#define DATA_DUMP_TIMER_INTERVAL     APP_TIMER_TICKS(25, 0)
void startUartXfer(){
	app_timer_start(data_dump_id, DATA_DUMP_TIMER_INTERVAL, NULL);
}
...
void ble_our_service_on_ble_evt(ble_os_t * p_our_service, ble_evt_t * p_ble_evt)
{
	...
	switch (p_ble_evt->header.evt_id)
	{
		...
		case BLE_GATTS_EVT_WRITE:
			//SEGGER_RTT_WriteString(0,"characteristic write occurred\n");
			//uint8_t dataWritten = p_ble_evt->evt.gatts_evt.params.write.data[0];
			//SEGGER_RTT_printf(0, "%d\n", dataWritten);
			if (p_ble_evt->evt.gatts_evt.params.write.data[0] == 13){
				SEGGER_RTT_WriteString(0,"preparing to dump data to RTT...\n");
				startUartXfer();
				readFRAMflag = 1;
			}
			if (p_ble_evt->evt.gatts_evt.params.write.data[0] == 5){
				FRAMflag = 1;
			}
			if (p_ble_evt->evt.gatts_evt.params.write.data[0] == 2){
				SEGGER_RTT_WriteString(0,"central device connected\n");
			}
			if (p_ble_evt->evt.gatts_evt.params.write.data[0] == 1){
				SEGGER_RTT_WriteString(0,"notifications ON\n");
			}
			if (p_ble_evt->evt.gatts_evt.params.write.data[0] == 0){
				SEGGER_RTT_WriteString(0,"notifications OFF\n");
			}
			break;
		default:
...
void our_termperature_characteristic_update(ble_os_t *p_our_service, uint8_t *temperature_value)
{
